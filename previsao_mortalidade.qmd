...
prev_comb = await FileAttachment('dados/arima-ets/previsao_concatenados.csv').csv()
prev_lm = await FileAttachment('dados/lc-lm/previoes_lm.csv').csv()
prev_lc = await FileAttachment('dados/lc-lm/previoes_lc.csv').csv()

modelos = ["ARIMA+ETS", "Lee-Carter", "Lee-Miller", "IBGE"]
...
data_filter1 = prev_comb.filter(d => d.regiao === regiao2 && d.Sexo === sexo).map(d => ({
  ano: +d.Ano,
  faixa: d.faixa_etaria,
...
{  
  if (modelo2.includes("ARIMA+ETS")) {
    ...
    return html`<p>Para acessar a metodologia utilizada no modelo Lee-Carter, <a href="metodologia.html#projeções-de-mortalidade-do-ibge">clique aqui</a>.</p>`;
  }
}
```
...
<body>
    ...
    <a href="dados/lc-lm/previoes_lc.csv" download>
        Lee-Carter.
    </a>
    </p>

    <p>Baixar previsões do modelo
    <a href="dados/lc-lm/previoes_lm.csv" download>
        Lee-Miller.
    </a>
    </p>
</body>
...
// Unir dados
data1 = [...erros_ae, ...erros_lc, ...erros_lm]
...
function createboxplot(data, metrica) {
  ...
  const x = d3.scaleBand()
    .domain(["ARIMA+ETS", "Lee-Carter", "Lee-Miller"])
    .range([margin.left, width - margin.right])
    .padding(0.3)
  ...
  const color = d3.scaleOrdinal()
    .domain(modelos) // Mapeia os nomes dos modelos...
    .range(['#e41a1c', '#377eb8', '#4daf4a']); // ...para as cores desejadas.
...
html`<p>Para acessar a métodologia das métricas de erro do modelo Arima+ETS <a href="metodologia.html#modelo-h%C3%ADbrido-arima-ets-com-pondera%C3%A7%C3%A3o-din%C3%A2mica">clique aqui</a>, para os modelos Lee-Carter e Lee-Miller <a href="metodologia.html#modelo-lee-carter">clique aqui</a></p>`
```

## Barras
```{ojs}
html`<h3>Métricas de erro por Grupo Etário em escala Logarítmica</h3>`
viewof erro_select2 = Inputs.select(["MAE", "MAPE", "RMSE"], {
  label: "Métrica de Erro",
  value: "MAE"
})
plotData2 = data1.map(d => ({
  modelo: d.modelo,
  valor: Math.log(d[erro_select2]),
  faixa: d.faixa
...
erros2 = await FileAttachment("dados/arima-ets/erros_concatenados.csv").csv({typed: true})
